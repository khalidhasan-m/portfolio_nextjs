...
export default function Home() {
  ...
  const featuredProjects = filteredProjects.filter((project) => Boolean(project.caseStudy));
  const supportingProjects = filteredProjects.filter((project) => !project.caseStudy);
  ...
  return <div className={`site-shell ${isDark ? "is-dark" : ""}`}>
    ...
    <main id="main-content">
      ...
      <section className="insights-section section-spine" id="insights"><div className="spine-label"><span>06</span><span>FIELD NOTES</span></div><div className="insights-content"><div className="insights-heading"><div><BrandMark className="insights-signature" /><p className="eyebrow">Notes from the build</p><h2>Small decisions,<br />made visible.</h2></div><p>Short principles drawn from the work itself—how interaction, operational clarity, and protected states make a product more useful.</p></div><div className="insights-list">{insights.map((insight) => <article key={insight.number}><span>{insight.number}</span><div><p>{insight.project} / implementation note</p><h3>{insight.title}</h3><p>{insight.text}</p></div></article>)}</div></div></section>
      ...
      <Suspense fallback={<section className="contact-section contact-loading" id="contact" aria-label="Loading the contact form" />}><ContactForm /></Suspense>
    </main>
